/**
 * A little promise timer
 *
 * @param {number} time How long to wait for
 */
...
export const clickOutside = (
	...
	const cb = (e) => {
		if (performance.now() - now < mountDebounce) {
			return;
		}
...
export const promiseRunner = (arr, cb) => {
	...
	return new Promise((resolve) => {
		const run = async (idx = 0) => {
			...
			data.push(await Promise.resolve().then(() => cb(arr[idx], idx, arr)));
